...
use super::{BufferState, CursorState, RimState};
...
impl RimState {
	...
	pub fn insert_char_at_cursor(&mut self, ch: char) {
		...
		let line = lines.get_mut(row_idx).expect("row index must exist after resize");
	...
	pub fn join_line_below_at_cursor(&mut self) {
		...
		let current = lines.get_mut(row_idx).expect("current row must exist while joining");
	...
	pub fn backspace_at_cursor(&mut self) {
		...
		if col_idx > 0 {
			...
			let prev_line = lines.get_mut(row_idx.saturating_sub(1)).expect("previous row must exist");
	...
	pub fn paste_slot_at_cursor(&mut self) {
		...
		buffer.cursor.col = buffer.cursor.col.saturating_add(slot_text.chars().count() as u16);
		buffer.text = lines.join("\n");
		self.mark_active_buffer_dirty();
		self.align_active_window_scroll_to_cursor();
		self.status_bar.message = "pasted".to_string();
	...
	pub fn delete_current_line_to_slot(&mut self) {
		...
		buffer.text = lines.join("\n");
		let visible_rows = if buffer.text.is_empty() { 1 } else { buffer.text.lines().count().max(1) };
	...
	pub fn delete_visual_selection_to_slot(&mut self) {
		...
		let deleted_text = if start_row == end_row {
			...
			let start_keep_byte = char_to_byte_idx(start_line, start_col.saturating_sub(1) as usize);
	...
	fn max_row(&self) -> u16 {
		self
			.active_buffer_text()
			.map(|text| if text.is_empty() { 1 } else { text.lines().count() as u16 })
			.unwrap_or(1)
	}
	...
	fn active_buffer_text(&self) -> Option<&str> {
		...
		self.buffers.get(buffer_id).map(|buffer| buffer.text.as_str())
	}

	fn active_buffer_cursor_mut(&mut self) -> Option<&mut CursorState> {
		let buffer_id = self.active_buffer_id()?;
		self.buffers.get_mut(buffer_id).map(|buffer| &mut buffer.cursor)
	}
	...
	fn active_window_visible_text_cols(&self) -> u16 {
		...
		self
			...
			.map(|window| {
				...
				let total_lines = self.active_buffer_text().map(|text| text.lines().count().max(1)).unwrap_or(1);
...
fn char_to_byte_idx(s: &str, char_idx: usize) -> usize {
	...
	s.char_indices().nth(char_idx).map(|(idx, _)| idx).unwrap_or(s.len())
}

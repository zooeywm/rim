...
use anyhow::Context;
use rim_kernel::{action::{AppAction, FileAction, FileLoadSource}, ports::{FileIo, FileIoError}, state::BufferId};
use tracing::error;
...
impl AsRef<FileIoState> for FileIoState {
	fn as_ref(&self) -> &FileIoState { self }
}
...
where Deps: AsRef<FileIoState>
{
	fn enqueue_load(&self, buffer_id: BufferId, path: PathBuf) -> Result<(), FileIoError> {
		self.request_tx.send(FileIoRequest::LoadFile { buffer_id, path, source: FileLoadSource::Open }).map_err(
			|err| {
				...
				FileIoError::RequestChannelDisconnected { operation: "load" }
			},
		)
	}
	...
	fn enqueue_save(&self, buffer_id: BufferId, path: PathBuf, text: String) -> Result<(), FileIoError> {
		self.request_tx.send(FileIoRequest::Save { buffer_id, path, text }).map_err(|err| {
			...
			FileIoError::RequestChannelDisconnected { operation: "save" }
		})
	}
	...
	fn enqueue_external_load(&self, buffer_id: BufferId, path: PathBuf) -> Result<(), FileIoError> {
		self
			...
			.map_err(|err| {
				...
				FileIoError::RequestChannelDisconnected { operation: "reload" }
			})
	}
}

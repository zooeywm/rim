use std::{hash::{Hash, Hasher}, ops::ControlFlow, path::PathBuf};

use crossterm::event::{KeyCode, KeyEvent, KeyModifiers};
use thiserror::Error;
use tracing::{error, info};

use crate::{action::{AppAction, BufferAction, EditorAction, FileAction, FileLoadSource, LayoutAction, SystemAction, TabAction, WindowAction}, file_io_service::{FileIo, FileIoServiceError, FileIoState}, file_watcher_service::{FileWatcher, FileWatcherServiceError, FileWatcherState}, state::{AppState, BufferEditSnapshot, BufferHistoryEntry, BufferId, BufferSwitchDirection, CursorState, EditorMode, FocusDirection, NormalSequenceKey, PendingInsertUndoGroup, SplitAxis}};
...
#[derive(Debug)]
enum PreEditCapture {
	Entry { buffer_id: BufferId, entry: BufferHistoryEntry },
}
...
where Deps: AsRef<ActionHandlerState>
		...
		+ FileWatcher
{
	...
	fn should_ignore_external_reload(state: &mut AppState, buffer_id: BufferId, text: &str) -> bool {
	...
	fn begin_insert_undo_group(state: &mut AppState) {
		...
		state.pending_insert_group =
			Some(PendingInsertUndoGroup { buffer_id, before_cursor: state.active_cursor(), edits: Vec::new() });
	}

	fn append_insert_undo_edit(state: &mut AppState, buffer_id: BufferId, edit: BufferEditSnapshot) {
	...
	fn commit_insert_undo_group(state: &mut AppState) {
		...
		let after_cursor =
			state.buffers.get(group.buffer_id).map(|buffer| buffer.cursor).unwrap_or(group.before_cursor);
		state.push_buffer_history_entry(group.buffer_id, BufferHistoryEntry {
			edits: group.edits,
	...
	fn dispatch_internal(&mut self, state: &mut AppState, action: AppAction) -> ControlFlow<()> {
		match action {
			...
			AppAction::File(FileAction::OpenRequested { path }) => {
				...
				if let Err(source) = self.prj_ref().enqueue_watch(buffer_id, normalized_path.clone()) {
	...
	fn capture_insert_mode_pre_edit(state: &AppState, key: KeyEvent) -> Option<(BufferId, BufferEditSnapshot)> {
		...
		match key.code {
			KeyCode::Char(ch) => {
				...
				Some((buffer_id, BufferEditSnapshot {
					start_byte,
			...
			KeyCode::Backspace => {
				if cursor.col > 1 {
					...
					let start = line_start + char_to_byte_idx(line, cursor.col.saturating_sub(2) as usize);
					let end = line_start + char_to_byte_idx(line, cursor.col.saturating_sub(1) as usize);
					Some((buffer_id, BufferEditSnapshot {
						start_byte:    start,
	...
	fn capture_open_line_below_pre_edit(state: &AppState) -> Option<(BufferId, BufferEditSnapshot)> {
		let buffer_id = state.active_buffer_id()?;
		let buffer = state.buffers.get(buffer_id)?;
		let cursor = buffer.cursor;
	...
	fn capture_open_line_above_pre_edit(state: &AppState) -> Option<(BufferId, BufferEditSnapshot)> {
		let buffer_id = state.active_buffer_id()?;
		let buffer = state.buffers.get(buffer_id)?;
		let cursor = buffer.cursor;
	...
	fn single_edit_entry(
		...
		BufferHistoryEntry {
			edits: vec![BufferEditSnapshot { start_byte, deleted_text, inserted_text }],
			before_cursor,
			after_cursor,
		}
	}

	fn capture_visual_selection_bounds(state: &AppState, text: &str) -> Option<VisualSelectionBounds> {
		...
		Some(VisualSelectionBounds { start, end, start_row, end_row })
	}
	...
	fn capture_cut_char_to_slot_pre_edit(state: &AppState) -> Option<(BufferId, BufferHistoryEntry)> {
		let buffer_id = state.active_buffer_id()?;
		let buffer = state.buffers.get(buffer_id)?;
		let cursor = buffer.cursor;
		let row_idx = cursor.row.saturating_sub(1) as usize;
	...
	fn capture_delete_current_line_pre_edit(state: &AppState) -> Option<(BufferId, BufferHistoryEntry)> {
		let buffer_id = state.active_buffer_id()?;
		let buffer = state.buffers.get(buffer_id)?;
		let cursor = buffer.cursor;
		let row_idx = cursor.row.saturating_sub(1) as usize;
		...
		let visible_rows = if after_text.is_empty() { 1 } else { after_text.lines().count().max(1) };
		let after_row = row_idx.min(visible_rows.saturating_sub(1)).saturating_add(1) as u16;
		let after_cursor = CursorState { row: after_row, col: 1 };

		let entry = Self::single_edit_entry(cursor, after_cursor, start, deleted_text, String::new());
		Some((buffer_id, entry))
	}

	fn capture_join_line_below_pre_edit(state: &AppState) -> Option<(BufferId, BufferHistoryEntry)> {
		let buffer_id = state.active_buffer_id()?;
		let buffer = state.buffers.get(buffer_id)?;
		let cursor = buffer.cursor;
		let row_idx = cursor.row.saturating_sub(1) as usize;
		...
		let after_cursor = CursorState { row: cursor.row, col: cursor.col.min(max_col).max(1) };

		let entry = Self::single_edit_entry(cursor, after_cursor, boundary, deleted_text, inserted_text);
		Some((buffer_id, entry))
	}

	fn capture_paste_after_cursor_pre_edit(state: &AppState) -> Option<(BufferId, BufferHistoryEntry)> {
		...
		if state.line_slot_line_wise {
			...
			let mut inserted_lines = slot_text.split('\n').map(ToString::to_string).collect::<Vec<_>>();
			...
			let (start_byte, inserted_text) = if insert_at < lines.len() {
				(line_start_byte_idx(&buffer.text, insert_at)?, format!("{}\n", inserted_joined))
			} else {
				(buffer.text.len(), format!("\n{}", inserted_joined))
			};
			let after_cursor = CursorState { row: insert_at as u16 + inserted_count, col: 1 };

			let entry = Self::single_edit_entry(cursor, after_cursor, start_byte, String::new(), inserted_text);
		...
		let after_cursor =
			CursorState { row: cursor.row, col: cursor.col.saturating_add(slot_text.chars().count() as u16) };

		let entry = Self::single_edit_entry(cursor, after_cursor, start_byte, String::new(), slot_text.clone());
		Some((buffer_id, entry))
	}

	fn capture_pre_edit_snapshot(state: &AppState, mode: EditorMode, key: KeyEvent) -> Option<PreEditCapture> {
		match mode {
	...
	fn capture_normal_mode_pre_edit_snapshot(state: &AppState, key: KeyEvent) -> Option<PreEditCapture> {
		let normal_key = Self::to_normal_key(state, key)?;
		let mut keys = state.normal_sequence.clone();
		keys.push(normal_key);

		let SequenceMatch::Action(AppAction::Editor(editor_action)) = Self::resolve_normal_sequence(&keys) else {
			return None;
		};
	...
	fn capture_visual_delete_pre_edit(state: &AppState) -> Option<(BufferId, BufferHistoryEntry)> {
		...
		if state.is_visual_line_mode() {
			let (start_byte, end_byte) =
				Self::visual_line_byte_range(&buffer.text, bounds.start_row, bounds.end_row, lines.len())?;
			...
			let after_text = lines.join("\n");
			let visible_rows = if after_text.is_empty() { 1 } else { after_text.lines().count().max(1) };
			let after_row = bounds.start_row.min(visible_rows.saturating_sub(1)).saturating_add(1) as u16;
			let after_cursor = CursorState { row: after_row, col: 1 };

			let entry =
				Self::single_edit_entry(before_cursor, after_cursor, start_byte, deleted_text, String::new());
			return Some((buffer_id, entry));
		}
		...
		if bounds.start_row == bounds.end_row {
			...
			let after_cursor = CursorState { row: bounds.start.row, col: start_col.min(new_len.saturating_add(1)) };
			let entry =
				Self::single_edit_entry(before_cursor, after_cursor, start_byte, deleted_text, String::new());
			return Some((buffer_id, entry));
		}
		...
		let merged_len = merged_prefix.chars().count() as u16 + merged_suffix.chars().count() as u16;
		let after_cursor =
			CursorState { row: bounds.start.row, col: start_col.min(merged_len.saturating_add(1)) };

		let entry = Self::single_edit_entry(before_cursor, after_cursor, start_byte, deleted_text, String::new());
		Some((buffer_id, entry))
	}
	...
	fn capture_visual_paste_pre_edit(state: &AppState) -> Option<(BufferId, BufferHistoryEntry)> {
		...
		if state.is_visual_line_mode() {
			let (start_byte, end_byte) =
				Self::visual_line_byte_range(&buffer.text, bounds.start_row, bounds.end_row, lines.len())?;
			let deleted_text = buffer.text[start_byte..end_byte].to_string();
			let mut replacement = slot_text.split('\n').map(ToString::to_string).collect::<Vec<_>>();
			if replacement.is_empty() {
				replacement.push(String::new());
			}
			let inserted_text = replacement.join("\n");
			let after_cursor = CursorState { row: bounds.start_row.saturating_add(1) as u16, col: 1 };
			let entry =
				Self::single_edit_entry(before_cursor, after_cursor, start_byte, deleted_text, inserted_text);
			return Some((buffer_id, entry));
		}
		...
		let deleted_text = buffer.text[start_byte..end_byte].to_string();
		let after_cursor = CursorState { row: bounds.start_row.saturating_add(1) as u16, col: start_col };
		let entry =
			Self::single_edit_entry(before_cursor, after_cursor, start_byte, deleted_text, slot_text.clone());
		Some((buffer_id, entry))
	}
		...
		match key.code {
			...
			KeyCode::Enter => {
				...
				match command.as_str() {
					"" => {}
					"qa" => {
						return self.dispatch_internal(state, AppAction::System(SystemAction::Quit));
					}
					"q!" | "quit!" => {
						if state.active_tab_window_ids().len() > 1 {
							return self.dispatch_internal(state, AppAction::Window(WindowAction::CloseActive));
						} else if state.tabs.len() > 1 {
							return self.dispatch_internal(state, AppAction::Tab(TabAction::CloseCurrent));
						} else {
							return self.dispatch_internal(state, AppAction::System(SystemAction::Quit));
						}
					}
					...
					_ if command.starts_with("w ") => {
						...
						if path.is_empty() {
							...
							self.enqueue_save_active_buffer(state, false, false, Some(PathBuf::from(path)));
						}
					}
					_ if command.starts_with("w! ") => {
						...
						if path.is_empty() {
							...
							self.enqueue_save_active_buffer(state, false, true, Some(PathBuf::from(path)));
						}
					}
					_ if command.starts_with("wq ") => {
						...
						if path.is_empty() {
							...
							self.enqueue_save_active_buffer(state, true, false, Some(PathBuf::from(path)));
						}
					}
					_ if command.starts_with("wq! ") => {
						...
						if path.is_empty() {
							...
							self.enqueue_save_active_buffer(state, true, true, Some(PathBuf::from(path)));
						}
					}
					_ => {
...
mod tests {
	...
	use crate::{action::{AppAction, BufferAction, EditorAction, LayoutAction, TabAction}, file_io_service::{FileIo, FileIoImpl, FileIoServiceError, FileIoState}, file_watcher_service::{FileWatcher, FileWatcherImpl, FileWatcherServiceError, FileWatcherState}, state::{AppState, BufferId}};
	...
	impl AsRef<ActionHandlerState> for TestHarness<'_> {
		fn as_ref(&self) -> &ActionHandlerState { &self.action_handler_state }
	}

	impl AsMut<ActionHandlerState> for TestHarness<'_> {
		fn as_mut(&mut self) -> &mut ActionHandlerState { &mut self.action_handler_state }
	}

	impl AsMut<AppState> for TestHarness<'_> {
		fn as_mut(&mut self) -> &mut AppState { &mut self.app_state }
	}

	impl AsRef<FileIoState> for TestHarness<'_> {
		fn as_ref(&self) -> &FileIoState { self.io }
	}

	impl AsRef<FileWatcherState> for TestHarness<'_> {
		fn as_ref(&self) -> &FileWatcherState { self.watcher }
	}

...
use crate::action::{AppAction, FileAction, FileLoadSource};
...
where
    Deps: AsRef<FileIoState>,
{
    fn enqueue_load(&self, buffer_id: BufferId, path: PathBuf) -> Result<(), FileIoServiceError> {
        self.request_tx
            .send(FileIoRequest::LoadFile {
                buffer_id,
                path,
                source: FileLoadSource::Open,
            })
            .map_err(|err| {
                error!(
    ...
    fn enqueue_external_load(
        ...
        self.request_tx
            .send(FileIoRequest::LoadFile {
                buffer_id,
                path,
                source: FileLoadSource::External,
            })
            .map_err(|err| {
                error!(
                    "enqueue_external_load failed: io request channel is disconnected: {}",
                    err
                );
                FileIoServiceError::RequestChannelDisconnected {
                    operation: "reload",
                }
            })
    }
                        ...
                        while let Ok(request) = request_rx.recv() {
                            match request {
                                FileIoRequest::LoadFile {
                                    ...
                                    .context(
                                        "failed to send LoadCompleted after runtime init failure",
                                    )?;
                                }
                                FileIoRequest::Save { buffer_id, .. } => {
                                    ...
                                    .context(
                                        "failed to send SaveCompleted after runtime init failure",
                                    )?;
                                }
                            }
                        }
                    ...
                    match request {
                        FileIoRequest::LoadFile {
                            ...
                            let result = runtime.block_on(async {
                                ...
                                let file_bytes =
                                    compio::fs::read(&path).await.with_context(|| {
                                        format!("read file failed: {}", display_path)
                                    })?;
                                String::from_utf8(file_bytes).with_context(|| {
                                    format!("decode utf-8 failed: {}", display_path)
                                })
                            });
                            event_tx
                                .send(AppAction::File(FileAction::LoadCompleted {
                        ...
                        FileIoRequest::Save {
                            ...
                            let result = runtime.block_on(async {
                                let display_path = path.display().to_string();
                                let write_result =
                                    compio::fs::write(&path, text.into_bytes()).await.0;
                                write_result
                                    .with_context(|| format!("write file failed: {}", display_path))
                                    .map(|_| ())
                            });
                            event_tx
                                .send(AppAction::File(FileAction::SaveCompleted {

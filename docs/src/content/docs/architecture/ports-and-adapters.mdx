...
flowchart LR
    ...
    Storage["rim-infra-storage"] --> Application
    Storage --> Domain["rim-domain"]
    Storage --> Ports
    Watcher["rim-infra-file-watcher"] --> Application
    Watcher --> Domain
    Watcher --> Ports
    UI["rim-infra-ui"] --> Application
    UI --> Domain
    Input["rim-infra-input"] --> Application
    App["rim-app"] --> Picker["host file picker"]
```
